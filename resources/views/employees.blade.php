                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($employees as $key => $emp)
                            <tr
                                class="{{ $key % 2 === 0 ? 'bg-white' : 'bg-gray-50' }} hover:bg-gray-100 transition-colors duration-200">
                                <td class="px-6 py-4 font-medium text-gray-800">{{ $emp->id }}</td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div
                                            class="h-10 w-10 flex-shrink-0 mr-3 bg-blue-100 rounded-full flex items-center justify-center">
                                            <span
                                                class="text-blue-800 font-semibold">{{ substr($emp->fullname, 0, 1) }}</span>
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-medium {{ $emp->gender == 'Male' ? 'bg-blue-100 text-blue-800' : ($emp->gender == 'Female' ? 'bg-pink-100 text-pink-800' : 'bg-purple-100 text-purple-800') }}">
                                        {{ $emp->gender }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="mailto:{{ $emp->email }}"
                                        class="text-blue-600 hover:underline">{{ $emp->email }}</a>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="tel:{{ $emp->tel }}"
                                        class="text-blue-600 hover:underline">{{ $emp->tel }}</a>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-medium {{ $emp->age < 30 ? 'bg-green-100 text-green-800' : ($emp->age > 50 ? 'bg-orange-100 text-orange-800' : 'bg-blue-100 text-blue-800') }}">
                                        {{ $emp->age }} years
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                ...
                <div class="flex justify-between items-center">
                    <div class="text-sm text-gray-700">
                        Showing <span class="font-medium">1</span> to <span
                            class="font-medium">{{ count($employees) }}</span> of <span
                            class="font-medium">{{ count($employees) }}</span> employees
                    </div>
                    <div class="flex space-x-1">
                        <span class="px-3 py-1 border border-gray-300 rounded-md bg-blue-500 text-white">1</span>
                        <span
                            class="px-3 py-1 border border-gray-300 rounded-md bg-white text-gray-700 hover:bg-gray-100 cursor-pointer">2</span>
                        <span
                            class="px-3 py-1 border border-gray-300 rounded-md bg-white text-gray-700 hover:bg-gray-100 cursor-pointer">3</span>
                        <span
                            class="px-3 py-1 border border-gray-300 rounded-md bg-white text-gray-700 hover:bg-gray-100 cursor-pointer">
